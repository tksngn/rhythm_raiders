...
- **背景画像のバイト削減（未実施）**: 0.5〜1.6MB の JPG が複数。当環境に画像再圧縮ツール（ImageMagick/sharp/cwebp）が無く未対応。オフラインで [squoosh.app] や `sharp` 等を使い、長辺〜1600px・品質80%程度へ再圧縮すると初回表示が軽くなる（特に無料枠のコールドスタート時）。
