...
- **マイページのレイアウト（2026-07-26）**: My Tracks が**曲数で伸びる唯一のカード**で、左列に置くと右列に余白が残る構造だった。My Tracks を**下段へ全幅で移動**し、上段に Profile（左）と Follow/Good Sound/CTA（右）を並べる形に変更（`.mypage__full { grid-column: 1 / -1 }`）。全幅だと1列では横に間延びするので曲は**2列グリッド**（768px以下は1列）。
