...
- **あわせて動画欄の文言を修正**: 「Neural Frames などで作った動画を…」という案内が*そのツールが前提*と読め、課金の壁で詰まる原因になっていた。実装はホストとID形式しか見ていないので「YouTube・Vimeoの動画URLであれば何でも埋め込めます（制作ツールは問いません）」に変更（投稿・編集の両フォーム）。
...
7. コントローラ経由の更新も4パターン確認: ①アップロード→添付される ②削除チェックのみ→外れる ③削除チェック＋新ファイル同時→**差し替えが優先** ④保存失敗（Title空欄）＋削除チェック→編集画面に戻り**既存画像は消えない**（purgeをupdate成功後にした狙いどおり）

### 表示の詰めと一覧サムネイル（2026-07-25 追加対応）

- **詳細ページ**: 縦長画像でカードが間延びしないよう `max-height: 420px` + `object-fit: contain` を追加。生成画像は全体を見せたいので cover(切り抜き)は使わない。編集画面のプレビューは `--preview` 修飾子で 220px に抑える。
- **一覧・マイページにサムネイル追加**: 共通パーシャル `layouts/_track_thumb` を新設し、楽曲一覧 / ゲスト一覧 / マイページの3箇所で使う。**一覧は正方形タイルに揃えたいので、詳細ページと違い `object-fit: cover`（切り抜き）**。画像なしの楽曲は **♪ のプレースホルダ**を同じ枠で出し、混在しても行の高さが崩れないようにした。
- **N+1対策（実測付き）**: 一覧は全件ぶん `music_image.attached?` を回すため preload を追加。あわせて元からあった `music_file`（プレーヤー）のN+1も解消し、**ActiveStorage関連クエリが 8回 → 4回**（曲数に依存しない定数）になった。
  - **マイページには `music_file` の preload を入れない**。プレーヤーを描画しないので、入れても使われず**実測で 4→6 回に悪化した**ため戻した。preload は「実際に使う関連だけ」。
- **詳細ページでの配置（2026-07-26 変更）**: 静止画・動画を**左カラムから右カラムの「♪ Music Index」ボタンの下**へ移した。左（制作者情報＋プレーヤー＋コメント投稿フォーム）が詰まる一方、右（コメント一覧）はコメントが少ないと大きく空くため、左右のバランスが悪かった。プレーヤーは左に残しており、右の動画は `window.wavesurfer_<id>` をID経由で参照するのでDOM位置が変わっても同期は壊れない（左が先に描画されるため初回のbindで繋がる）。
  - **副作用**: コメントが多い楽曲では、メディアがコメント一覧より下に押し下げられる。気になるならコメント一覧の**上**に置く形に変えられる。
- **動画の縦横比（2026-07-26）**: `.ts-mv__frame` が 16:9 固定だったため、Suno の出力のような正方形・縦長の動画で**左右に大きな黒帯**が出ていた。アップロード動画には `--file` 修飾子を付け、**元の縦横比のまま幅いっぱい**に表示する（`max-height: 70vh` で上限だけ設ける）。YouTube/Vimeo の iframe は内在比を持たないため 16:9 のまま。
- **マイページのレイアウト（2026-07-26）**: My Tracks が**曲数で伸びる唯一のカード**で、左列に置くと右列に余白が残る構造だった。My Tracks を**下段へ全幅で移動**し、上段に Profile（左）と Follow/Good Sound/CTA（右）を並べる形に変更（`.mypage__full { grid-column: 1 / -1 }`）。全幅だと1列では横に間延びするので曲は**2列グリッド**（768px以下は1列）。ただし**1曲しかないときは2列の片側が空くだけ**なので、`@created_tracks.size < 2` のとき `mp-track-list--few` を付けて1列に戻す。
  - **ページネーション（2026-07-26）**: My Tracks を **5曲ずつ**にし、6曲目からページ送りが出るようにした（kaminari は総ページ数が1なら何も描画しないので、5曲以下では何も出ない）。
  - **ハマった点**: 最初 `@member.created_tracks.page(...).per(5)` と書いて **`undefined method 'per'` で 500**。関連経由だと `AssociationRelation` になり `per` が生えない（CLAUDE.md に既出の注意そのもの）。`CreatedTrack.page(params[:page]).per(5).where(member_id: ...)` と**モデル直**で書いて解決。
  - 検証: 1曲→5曲→6曲(1/2ページ)と実際に増減させ、5曲まではページ送りが出ず、6曲でページ2へのリンクが出る／2ページ目は1曲のため1列になる、までを確認。
- **検証**: 画像あり1件・なし2件が混在する状態で、楽曲一覧 / ゲスト一覧 / マイページ / 楽曲詳細 / 編集 の5画面すべて **200**。一覧はサムネ枠3・プレースホルダ2で出し分けを確認。クエリ数は3ページとも4回。

---

## 4.10 AI動画のファイル直接アップロード（2026-07-25）

- **背景**: 制作ツールが Neural Frames → **Suno.AI** に変わり、手元に mp4 が出力されるようになった。YouTube/Vimeo に上げてURLを貼る手順が余計なので、ファイルを直接アップロードできるようにした。
- **方式**: `has_one_attached :music_video_file`。**MP4 / WebM・50MBまで**（`VIDEO_MAX_SIZE`。Supabase 無料枠の1ファイル上限に合わせた値）。超える動画は従来の `music_video_url`（YouTube/Vimeo埋め込み）に誘導する。**両方を併存**させており、URL欄は残してある。
- **配信は redirect 方式**（`rails_storage_redirect_path`）。音源プレーヤーは `rails_storage_proxy_path` を使っていてファイルの中身が Render を経由するが、**動画で同じことをすると無料インスタンスを直撃する**ため、動画はブラウザから Supabase を直接読ませる。
- **音の二重を避ける**: 詳細ページの `<video>` は **muted + loop**。音は従来どおり wavesurfer で鳴らす。`controls` は残してあるので、動画側の音を聴きたい人は解除できる。
- **波形プレーヤーと同期**: wavesurfer の `play` / `pause` / `finish` を購読して映像を追従させる。プレーヤーの初期化がこのスクリプトより後になり得るため、`window.wavesurfer_<id>` に繋がるまで 200ms 間隔で最大20回リトライする。
- **削除処理を共通化**: 対象が画像・動画の2つになったので、`update` の purge をループにまとめた。**purge は update 成功後**・**差し替えは削除指定より優先**という方針は従来どおり。

### 検証（ローカル Docker・2026-07-25）

- バリデーション: mp4 → OK / png → 「MP4またはWebMを選んでください」/ 60MB → 「50MB以下に…（大きい動画はURL欄を）」（blobの `byte_size` を書き換えて境界を確認）
- 画面: 詳細 200（`<video>` 1個・muted・同期スクリプトあり）/ 編集 200（プレビューと削除チェック）/ 投稿 200 / 一覧 200
- コントローラ経由 7パターン: ①画像+動画を同時アップ ②動画だけ削除（画像は残る）③画像だけ削除 ④再アップ ⑤両方まとめて削除 ⑥削除+差し替え同時 → **差し替えが優先** ⑦保存失敗(Title空欄)+削除チェック → **添付は消えない**

### 一覧サムネイルが「出ない」と見えた件（2026-07-25）

- **症状**: 一覧・マイページで画像が出ず、♪ プレースホルダのまま。
- **切り分け**（本番の実物を取りにいくのが速い）:
  1. 本番の配信CSSを直接取得 → `.track-thumb` 等が含まれる＝**デプロイは済んでいる**
  2. READMEのデモ会員でログインして本番HTMLを取得 → **全7曲すべて ♪、ActiveStorage の img はゼロ**
  3. 各詳細ページも確認 → **どの曲にも `ts-artwork` が無い**＝静止画は1件も添付されていなかった
- **真因**: アップロードされていたのは静止画ではなく **mp4**（track 8「空の向こうへ」に `<video id="mv_8">`）。サムネイルは `music_image` しか見ていなかったため ♪ のままだった。**コードのバグではなく、静止画と動画で見ている属性が違っただけ**。
- **対応**: 静止画が無く動画がある場合は **動画の先頭フレームをサムネイルに使う**ようにした。`preload="metadata"` なので動画全体はDLしない。`#t=0.1` を付けるのは、0秒ちょうどだと真っ黒のフレームになる動画があるため。`music_video_file_attachment` の preload も追加（一覧6クエリ・マイページ5クエリで曲数に非依存）。
- **教訓**: 「表示されない」は CSS → HTML → データ の順に、**本番の実物を curl で取って**潰すと速い。ログインが要る画面もデモ会員の認証情報で取得できる。今回は「コードが悪い」と決め打ちせずデータを見にいったことで、**そもそも静止画が1件も無い**という真因に辿り着けた。
- **結果**: 2026-07-26、本番の一覧・マイページにサムネイルが表示されることをユーザーが確認済み。

### 本番確認と注意

- **実ファイルでの再生・シークを確認済み（2026-07-26）**。ローカル検証は中身がダミーのファイルで行ったため、**シーク（再生位置移動）が Supabase の signed URL 経由で効くか**が最後まで未確認だったが、本番で実際の mp4 の再生バーをドラッグして正しく飛べることをユーザーが確認した。redirect 方式（ブラウザ →Supabase 直接）で range リクエストが機能している。
- **無料枠の消費に注意**。Supabase 無料はストレージ1GB・帯域にも上限があり、mp4は音源の10倍規模になりやすい。数本で枯渇し得るので、長い動画はURL欄（YouTube）に寄せる運用が無難。

---

## 4.11 favicon（タブのアイコン）を設定（2026-07-26）
...
## 4.12 楽曲詳細にループ再生を追加（2026-07-26）

- 波形の下に **🔁 LOOP OFF / ON のトグルボタン**を追加。ONだと末尾まで再生後に頭から流し直す（wavesurfer の `finish` を購読して `stop()` → `play()`。既存の「Start Over」と同じ手順なので確実に動く形を踏襲）。
- **楽曲詳細だけに出す**。`layouts/_player` に `show_loop` ローカルを追加し、`created_tracks#show` からのみ `show_loop: true` を渡す。一覧は1画面に複数プレーヤーが並ぶため対象外。**ループ用のJS（状態変数・トグル関数・finishハンドラ）も `show_loop` の時だけ出力**し、一覧に不要なコードを吐かない。
- **動画との整合が要点**: 詳細ページのmp4は wavesurfer の再生/停止に追従する作りで、`finish` で映像を止めていた。ループONだと**音だけ続いて映像が止まる**ため、ループ中は映像を止めない判定を追加。ハンドラの登録順に依存しないよう、**映像側でも `window.wsLoop_<id>` を見て**判断する（プレーヤーは左カラム、動画は右カラムにあり登録順が入れ替わり得るため）。
- **検証**: 詳細=LOOPボタン1個・トグル関数1個・`wsLoop_` 参照5箇所、一覧/ゲスト一覧=**ループ関連の出力ゼロ**（プレーヤー自体は3個）を確認。ただし**実際に音がループするかはブラウザ目視が必要**。
...
- **動画は「ファイルを直接アップロード（50MBまで）」と「YouTube/VimeoのURL」の2通り**（§4.10）。大きい動画は後者に寄せる。ストレージは Supabase 無料枠（1GB）なので mp4 の入れすぎに注意。
- **楽曲の編集は Title / Genre / Creator Word / AI静止画 / AI動画ファイル / AI動画URL**（§4.8・§4.9・§4.10）。音源ファイル(`music_file`)の差し替えだけは意図的に塞いでいる（いいね・コメントを保ったまま中身がすり替わるのを防ぐため）。
- AI静止画は楽曲詳細（大きく表示）と、一覧・ゲスト一覧・マイページ（正方形サムネイル）に出る（§4.9）。未設定の楽曲は ♪ のプレースホルダ。
